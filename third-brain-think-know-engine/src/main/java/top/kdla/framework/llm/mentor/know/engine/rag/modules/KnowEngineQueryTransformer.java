 ...
 * @see top.kdla.framework.llm.mentor.know.engine.chat.memory.DatabaseChatMemoryStore
 */
@Slf4j
public class KnowEngineQueryTransformer implements QueryTransformer {
    ...
    public Collection<Query> transform(Query query) {
        ...
        String llmResponse = chatModel.chat(createPrompt(query, format(chatMemory)).text());
        log.info("问题改写完成, 改写结果 : {} ,耗时: {}", llmResponse, stopwatch.stop().elapsed(TimeUnit.MILLISECONDS));

        // LLM 返回空时回退原始问题，避免构造非法 Query
        String newQuery = (llmResponse == null || llmResponse.isBlank()) ? query.text() : llmResponse;
